...
fn sync_dictionary(conn: &mut Connection, attach_sql: &str, src_version: i32) -> SqliteResult<()> {
    ...
    tx.execute(
        "INSERT INTO wo_sudachi_normal SELECT * FROM bundled_db.wo_sudachi_normal",
        [],
    )?;
    tx.execute(
        "INSERT INTO wo_sudachi_sahen SELECT * FROM bundled_db.wo_sudachi_sahen",
        [],
    )?;
...
// -----------------------------------------------------------------------------
// クロージャの中身を独立した関数に切り出しました
// 戻り値の型を `Result<(), Box<dyn Error>>` にするのがポイントです
// -----------------------------------------------------------------------------
        ...
        match app.path().resolve(resource_path, BaseDirectory::Resource) {
            Ok(db_src_path) => {
                if db_src_path.exists() {
                    fs::copy(&db_src_path, &db_dest_path)
                        .expect("データベースのコピーに失敗しました");
            ...
            if src_version > dest_version {
                ...
                match db_src_path.to_str() {
                    Some(src_path_str) => {
                        ...
                        let attach_sql =
                            format!("ATTACH DATABASE '{}' AS bundled_db", escaped_path);
